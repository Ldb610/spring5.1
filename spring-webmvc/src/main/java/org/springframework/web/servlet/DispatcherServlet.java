...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		...
		// lookupres 初始化HandlerMappings
		initHandlerMappings(context);
		// lookupres 初始化HandlerAdapters
	 ...
	 */
	// lookupRes 初始化该类使用的HandlerMappings。<p>如果在BeanFactory中没有为这个名称空间定义HandlerMapping bean，
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {
			...
			// lookupRes 父级容器和本容器都会去查找 是否有实现 HandlerMapping.class 的类
			Map<String, HandlerMapping> matchingBeans =
					BeanFactoryUtils.beansOfTypeIncludingAncestors(context, HandlerMapping.class, true, false);
			if (!matchingBeans.isEmpty()) {
				this.handlerMappings = new ArrayList<>(matchingBeans.values());
				// We keep HandlerMappings in sorted order.
				// lookupRes 我们保持HandlerMappings处于有序状态。
				// lookupRes 可以使用 @Order 注解进行排序
	...
	private void initHandlerExceptionResolvers(ApplicationContext context) {
		...
		if (this.detectAllHandlerExceptionResolvers) {
			...
			// lookupres 找到ApplicationContext中的所有HandlerExceptionResolvers，包括祖先上下文。
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// lookupRes springmvc 的核心方法，开始请求调度
	 ...
	 */
	//lookupRes 处理实际调度到controller。<p>处理程序将通过应用servlet的HandlerMappings来获得。
	// HandlerAdapter将通过查询servlet已安装的HandlerAdapter来获得，以找到第一个支持处理程序类的HandlerAdapter。
	// <p>所有的HTTP方法都由这个方法处理。由handleradapter或处理程序本身决定哪些方法是可接受的。
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				// lookupRes 检查是否是文件请求，如果是的话把Request 改成文件请求的request
	 ...
	 */
	// lookupRes handlerMappings 是在webapplicationContext 容器初始化的时候初始化的
	@Nullable
